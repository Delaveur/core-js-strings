...
function isPalindrome(str) {
  const reverse = str.replace(/[\W_]/g, '').split('').reverse().join('');
  if (reverse.toLowerCase() === str.replace(/[\W_]/g, '').toLowerCase()) {
    return true;
  }
  return false;
}
...
function invertCase(str) {
  ...
  str.split('').forEach((letter) => {
    if (letter.charCodeAt(0) <= 90) {
      invert += letter.toLowerCase(0);
    } else {
      invert += letter.toUpperCase(0);
    }
  });
  return invert;
}
